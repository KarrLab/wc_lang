...
# name of the singleton, root obj_model.Model in wc_lang.core
...
def get_root_model(models):
    ...
    """
    return get_solitary_class(models, ROOT_MODEL_NAME)


# todo: move to wc_utils
def get_solitary_class(classes, class_name):
    """ Get a solitary class from an iterator over multiple classes

    Args:
        classes (:obj:`iterator`): an iterator over some classes
        class_name (:obj:`str`): the solitary class' name

    Returns:
        :obj:`type`: a solitary class in `classes` whose name (`__name__`) is `class_name`

    Raises:
        :obj:`ValueError`: if `classes` doesn't contain exactly 1 class whose name is `class_name`
    """
    matching_classes = [cls for cls in classes if cls.__name__ == class_name]
    if len(matching_classes) != 1:
        raise ValueError("'classes' should contain exactly 1 class whose name is '{}'".format(class_name))
